...
TorBots sets two goals for every season, in priority order:

- **Threshold goal:** qualify for and compete at the Southern California District Championship (DCMP).
- **Objective goal:** qualify for and attend the FIRST Championship (Worlds).

The threshold goal is the bar the season is not considered successful without clearing. The objective goal is the stretch outcome that ambitious design and execution choices should be aimed at, without putting the threshold goal at risk.

## Design for the Whole Team, Not Just the Robot
...
Prefer mechanisms the team deeply understands and can execute consistently over novel mechanisms the team would be learning for the first time under competition pressure — unless the team genuinely has the student depth *and* mentor support to take that on this season.
